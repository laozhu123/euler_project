...
def is_triangle_words(word):
    return judge(sum(ord(word[i]) - ord('A') + 1 for i in range(len(word)) if True))


def judge(num):
    x = int(math.sqrt(num * 2))
    return x * (x + 1) == num * 2 or (x + 1) * (x + 2) == num * 2

def compute():
    ans = sum(1 for i in range(len(words)) if is_triangle_words(words[i]))
    return ans
